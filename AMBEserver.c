...
static const char DV3K_CONTROL_VERSTRING_RESPONSE[] = {0x56, 0x31, 0x32, 0x30, 0x2e, 0x45, 0x31, 0x30, 0x30, 0x2e, 0x58, 0x58, 0x58, 0x58, 0x2e, 0x43, 0x31, 0x30, 0x36, 0x2e, 0x47, 0x35, 0x31, 0x34, 0x2e, 0x52, 0x30, 0x30, 0x39, 0x2e, 0x42, 0x30, 0x30, 0x31, 0x30, 0x34, 0x31, 0x31, 0x2e, 0x43, 0x30, 0x30, 0x32, 0x30, 0x32, 0x30, 0x38, 0x00};
...
void dump(char *text, struct dv3k_packet *packet)
{
    unsigned int offset = 0U;
    unsigned int i;
    unsigned char *data = (unsigned char *) packet;
    unsigned int length = ntohs(packet->header.payload_length) + 4;
...
int initDV3K(int fd, int hwReset)
{
    ...
    if(checkResponse(&responsePacket, DV3K_CONTROL_PRODID[FIELD_ID]) == 0) {
       fprintf(stderr, "AMBEserver: invalid response to product id query\n");
       dump("ProdID Response:", &responsePacket);
       return 0;
    }
    ...
    if(checkResponse(&responsePacket, DV3K_CONTROL_VERSTRING[FIELD_ID]) == 0) {
...
int processSerial(int sockFd, int serialFd)
{
    ...
    if (debug)
        dump("Serial data", &packet);

    if (override_ver) {
        if (checkResponse(&packet, DV3K_CONTROL_PRODID[FIELD_ID]) == 1) {

           strncpy(packet.payload.ctrl.data.prodid, DV3K_CONTROL_PRODID_RESPONSE, sizeof(packet.payload.ctrl.data.prodid));

            if (debug) {
                fprintf(stderr, "AMBEserver: Received PRODID response\n");
                dump("PRODID Response:", &packet);
            }
        } else if (checkResponse(&packet, DV3K_CONTROL_VERSTRING[FIELD_ID]) == 1) {
           
           strncpy(packet.payload.ctrl.data.version, DV3K_CONTROL_VERSTRING_RESPONSE, sizeof(packet.payload.ctrl.data.version));

            if (debug) {
                fprintf(stderr, "AMBEserver: Received VERSTRING response\n");
                dump("VERSTRING Response:", &packet);
            }
        }
    }
...
void usage() {
#ifdef __CYGWIN__
    fprintf(stderr, "Usage: AMBEserver [-d] [-r] [-s speed] [-p port] [-c comport_number] [-v] [-x] [-o]\n\n");
    fprintf(stderr, "-d Daemon Mode\n");
    fprintf(stderr, "-r Forces a hardware reset using a GPIO pin. Intended for a 'hat' on a Raspberry Pi\n");
    fprintf(stderr, "-s Baud rate (Defaults to 230400)\n");
    fprintf(stderr, "-p UDP port to listen on (Defaults to 2460)\n");
...
#ifdef __CYGWIN__
    while ((c = getopt(argc, argv, "dp:s:c:vxrho")) != -1) {
#else
    while ((c = getopt(argc, argv, "dp:s:i:vxrho")) != -1) {
#endif
        switch (c) {
            case 'd':
                daemon = 1;
